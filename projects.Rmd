...
* +1 for explaining the color mapping
...
names <- names %>% 
  ...
  mutate(team = 1:n()) %>% 
  mutate(
    Member.1 = as.character(Member.1),
    Member.2 = as.character(Member.2),
    Member.1 = ifelse(Member.2 == "Jane Bang", "Xiruo Zheng", Member.1 ),
    Member.2 = ifelse(Member.1 == "Rachel Yan", "Ana Porro", Member.2 )
  ) %>% 
  filter(Member.2 != "Max Pabilonia") %>% 
