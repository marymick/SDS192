...
* Why do we care about "tidy" data format?
    + Short answer: Because all the functions in the `ggplot2` and `dplyr` R packages for data visualization and data wrangling *assume* that the input data is in "tidy" format and all output data are "tidy" as well. *This is for consistency*.
...
*Assigned Tuesday 10/2, due Monday 10/8 at 9pm Eastern Time*
...
library(dplyr)
set.seed(76)
poll_results <- data_frame(
  heard_of_imposter_syndrome = 
    c(rep("yes", times = 8 + 8 + 6 +12), rep("no", times = 3 + 2 + 4 + 2)),
  year = c(
    rep("senior", times = 8),
    rep("junior", times = 8),
    rep("sophomore", times = 6),
    rep("first-year", times = 12),
    rep("senior", times = 3),
    rep("junior", times = 2),
    rep("sophomore", times = 4),
    rep("first-year", times = 2)
  )
) %>% 
  sample_frac(1)
```
